...
export default function PositionsCreate({ organizations }: { organizations: Array<{ id: string; name_en: string }> }) {
    const { t } = useLocale();
    const form = useForm({
        job_position_code: '',
        title_en: '',
        ...
        <AuthenticatedLayout header={<PageHeader title={t('positions.createPosition')} />}>
            ...
            <form className="space-y-6 rounded-2xl border border-gray-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900" onSubmit={submit}>
                ...
                <div className="flex gap-3">
                    ...
                    <Link href={route('positions.index')} className="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 dark:border-slate-700 dark:text-slate-200">{t('common.cancel')}</Link>
                </div>
            </form>
        </AuthenticatedLayout>
    );
