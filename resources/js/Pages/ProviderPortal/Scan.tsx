...
type CalendarDay = {
    date: string; day_name: string; is_today: boolean; is_working_day: boolean;
    is_open: boolean; is_subsidy_day: boolean; is_public_holiday: boolean;
    is_special_day: boolean; is_employee_excluded: boolean; is_consumed: boolean;
    consumed_by_transaction_id: string | null; is_available: boolean;
    reason_code: string; label: string;
};

type TodayScan = {
    id: string; scanned_at: string | null; status: string | null; usage_mode: string | null;
    subsidy_amount_applied: number; employee_payable_amount: number;
    consumed_days_count: number; is_extra_scan: boolean;
    employee: { display_name: string; employee_number: string; photo_url: string | null; organization_name: string | null; organization_unit_name: string | null; position_title: string | null } | null;
};
...
type ScanResult = {
    ...
    available_days_count: number | null; consumed_days_count: number | null;
    remaining_after: number | null; week_start: string | null; week_end: string | null;
    consumed_dates?: string[]; calendar_days?: CalendarDay[];
    employee_id?: string | null; transaction_id?: string | null; duplicate?: boolean;
};

const cameraScanConfig: Html5QrcodeCameraScanConfig = {
    fps: 10,
    qrbox: (w: number, h: number) => { const e = Math.floor(Math.min(w, h) * 0.72); return { width: e, height: e }; },
    aspectRatio: 1, disableFlip: false,
};
...
function getMonthLabel(year: number, month: number, locale: string, isEthiopian: boolean): string {
    ...
    return new Intl.DateTimeFormat(locale === 'am' ? 'am-ET' : 'en', { month: 'long', year: 'numeric' }).format(new Date(year, month, 1));
}
...
function buildCalendarCells(year: number, month: number, isEthiopian: boolean): (CalCell | null)[] {
    if (isEthiopian) {
        ...
        for (let d = 1; d <= total; d++) cells.push({ day: d, gregorianIso: ethiopianToGregorianIso(year, month, d) ?? '' });
        return cells;
    }
    const firstDow = new Date(year, month, 1).getDay();
    const total = new Date(year, month + 1, 0).getDate();
    const cells: (CalCell | null)[] = Array(firstDow).fill(null);
    for (let d = 1; d <= total; d++) cells.push({ day: d, gregorianIso: isoDate(year, month, d) });
    return cells;
}
...
function computeCoveredDates(scanDate: string, result: ScanResult): string[] {
    ...
    if (result.usage_mode === 'use_remaining_week' && result.week_end && result.week_end >= scanDate) {
        ...
        while (curr <= end) { dates.push(curr.toISOString().slice(0, 10)); curr.setDate(curr.getDate() + 1); }
        return dates;
    }
    return [scanDate];
}
...
export default function ProviderPortalScan({
    ...
    const [calendarMeta, setCalendarMeta] = useState<CalendarDay[]>(scan_result?.calendar_days ?? []);
    ...
    const SCAN_HISTORY_KEY = 'cafeteria_portal_scan_history';
    const [scanHistory, setScanHistory] = useState<{ ts: string; isExtra: boolean; coveredDates: string[] }[]>(() => {
        try {
            ...
            return parsed.filter(e => e.ts.slice(0, 10) >= cutoffStr).map(e => ({ ...e, coveredDates: e.coveredDates ?? [e.ts.slice(0, 10)] }));
        } catch { return []; }
    });

    useEffect(() => {
        try { localStorage.setItem(SCAN_HISTORY_KEY, JSON.stringify(scanHistory)); } catch { }
    }, [scanHistory]);
    ...
    const getCameraErrorMessage = useCallback((error: unknown) => {
        ...
        if (error instanceof DOMException) {
            if (error.name === 'NotAllowedError') return t('cafeteria.cameraPermissionDenied');
            if (error.name === 'NotFoundError') return t('cafeteria.noCameraFound');
            if (error.name === 'NotReadableError') return t('cafeteria.cameraInUse');
            return error.message || t('cafeteria.cameraUnavailable');
        }
        return error instanceof Error ? error.message : t('cafeteria.cameraUnavailable');
    }, [t]);
    ...
    const submitScannedToken = useCallback((qrToken: string) => {
        ...
        router.post(route('provider.portal.scan.store'), {
            qr_token: token,
    ...
    const startCamera = useCallback(async () => {
        ...
        if (existing) { try { if (existing.isScanning) await existing.stop(); existing.clear(); } catch { } finally { scannerRef.current = null; setCameraActive(false); } }
    ...
    }, [cameraActive, cameraProcessing, getCameraErrorMessage, scannerRegionId, selectPreferredCamera, submitScannedToken, t]);

    function submit(e: FormEvent) {
        e.preventDefault();
        form.post(route('provider.portal.scan.store'), {
            preserveScroll: true,
    ...
    useEffect(() => {
        ...
        let count = 3;
        setCountdown(count);
        const iv = setInterval(() => { count--; setCountdown(count > 0 ? count : null); if (count <= 0) clearInterval(iv); }, 1000);
        const to = setTimeout(() => void startCamera(), 3000);
    ...
    useEffect(() => {
        ...
        if (scan_result?.allowed) {
            if (scan_result.calendar_days) setCalendarMeta(scan_result.calendar_days);
            const scanDate = new Date().toISOString().slice(0, 10);
            setScanHistory(prev => [...prev, { ts: new Date().toISOString(), isExtra: scan_result.is_extra_scan, coveredDates: computeCoveredDates(scanDate, scan_result) }]);
        }
    }, [scan_result]);

    useEffect(() => {
        let cancelled = false;
        void window.axios.get<{ calendar_days: CalendarDay[] }>(route('provider.portal.scan.calendar'), {
            params: { employee_id: scan_result?.employee_id ?? undefined, date: todayDateStr },
        }).then(r => { if (!cancelled) setCalendarMeta(r.data.calendar_days); });

        void window.axios.get<{ data: TodayScan[] }>(route('provider.portal.scan.today')).then(r => {
            if (!cancelled) setTodayScans(r.data.data);
        });

        return () => { cancelled = true; };
    }, [scan_result?.employee_id, todayDateStr]);

    useEffect(() => {
        const today = new Date();
        if (isEthiopian) { const eth = gregorianToEthiopian(today.getFullYear(), today.getMonth() + 1, today.getDate()); setCalYear(eth.year); setCalMonth(eth.month); }
    ...
    const scannedDateMap = scanHistory.reduce<Map<string, number>>((acc, { coveredDates }) => {
        for (const d of coveredDates) acc.set(d, (acc.get(d) ?? 0) + 1);
        return acc;
    }, new Map());

    const dayLabels = isEthiopian ? ['እሁ','ሰኞ','ማክ','ረቡ','ሐሙ','ዓር','ቅዳ'] : ['Su','Mo','Tu','We','Th','Fr','Sa'];
    const cells = buildCalendarCells(calYear, calMonth, isEthiopian);

    function prevMonth() {
        if (isEthiopian) { if (calMonth === 1) { setCalYear(y => y - 1); setCalMonth(13); } else setCalMonth(m => m - 1); }
        else { if (calMonth === 0) { setCalYear(y => y - 1); setCalMonth(11); } else setCalMonth(m => m - 1); }
    }
    function nextMonth() {
        if (isEthiopian) { if (calMonth === 13) { setCalYear(y => y + 1); setCalMonth(1); } else setCalMonth(m => m + 1); }
        ...
        <CafeteriaProviderPortalLayout title={t('providerPortal.scan')} providers={providers} selectedProviderId={selected_provider_id}>
            <form onSubmit={submit} className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
                ...
                <div className="border-b border-gray-200 px-6 py-4 dark:border-slate-800">
                    <h2 className="text-base font-semibold text-gray-900 dark:text-white">{t('cafeteria.scanQr')}</h2>
                </div>

                {/* Scan result banner */}
                {scan_result && (
                    <div className={`border-b px-6 py-4 ${scan_result.allowed ? (scan_result.is_extra_scan ? 'border-orange-200 bg-orange-50 dark:border-orange-900 dark:bg-orange-950/30' : 'border-emerald-200 bg-emerald-50 dark:border-emerald-900 dark:bg-emerald-950/30') : 'border-red-200 bg-red-50 dark:border-red-900 dark:bg-red-950/30'}`}>
                ...
                <div className="grid gap-6 p-6 lg:grid-cols-3 lg:items-start">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-sm font-medium text-gray-700 dark:text-slate-300">{t('cafeteria.enterQrToken')}</label>
                            <input className={inputCls} placeholder="xxxx-xxxx|token" value={form.data.qr_token} onChange={e => form.setData('qr_token', e.target.value)} />
                            {form.errors.qr_token && <p className="text-xs text-red-600">{form.errors.qr_token}</p>}
                        </div>
                        ...
                        {scan_result?.allowed && scan_result.week_start && (
                            <div className="rounded-xl border border-orange-200 bg-orange-50 p-4 dark:border-orange-900/40 dark:bg-orange-950/20">
                                ...
                                <dl className="space-y-1 text-sm">
                                    <div className="flex justify-between"><dt className="text-gray-500">{t('cafeteria.subsidyApplied')}</dt><dd className="font-semibold text-emerald-600">{scan_result.subsidy_applied?.toFixed(2)}</dd></div>
                                    {(scan_result.employee_payable ?? 0) > 0 && <div className="flex justify-between"><dt className="text-gray-500">{t('cafeteria.employeePayableAmount')}</dt><dd className="font-semibold text-orange-600">{scan_result.employee_payable?.toFixed(2)}</dd></div>}
                                    <div className="flex justify-between"><dt className="text-gray-500">{t('cafeteria.consumedDays')}</dt><dd className="font-medium">{scan_result.consumed_days_count} / {scan_result.available_days_count}</dd></div>
                                    <div className="flex justify-between border-t border-orange-200 pt-1 dark:border-orange-900/40"><dt className="font-medium text-gray-600">{t('cafeteria.remainingWeekBalance')}</dt><dd className="font-bold text-orange-700">{scan_result.remaining_after?.toFixed(2)}</dd></div>
                                </dl>
                            </div>
                        )}
                    </div>
                        ...
                        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
                            <div className="border-b border-gray-100 bg-gray-50/70 px-4 py-3 dark:border-slate-800 dark:bg-slate-800/40">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="text-center">
                                        <p className="text-sm font-semibold text-gray-900 dark:text-white">{getMonthLabel(calYear, calMonth, locale, isEthiopian)}</p>
                                        <span className={`text-[10px] font-medium ${isEthiopian ? 'text-orange-500' : 'text-gray-400'}`}>{isEthiopian ? (locale === 'am' ? 'የኢትዮጵያ ቀን አቆጣጠር' : 'Ethiopian Calendar') : (locale === 'am' ? 'ጎርጎሪያን ቀን አቆጣጠር' : 'Gregorian Calendar')}</span>
                                    </div>
                                    <button type="button" onClick={nextMonth} className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-500 hover:bg-white hover:shadow-sm dark:text-slate-400 dark:hover:bg-slate-700">
                            ...
                            <div className="px-3 pb-3 pt-2">
                                ...
                                <div className="grid grid-cols-7">
                                    {cells.map((cell, idx) => {
                                        ...
                                        let cellCls: string;
                                        if (dayMeta?.is_consumed) cellCls = 'bg-emerald-500 text-white font-bold shadow-sm';
                                        else if (dayMeta?.is_employee_excluded) cellCls = 'bg-red-100 text-red-600 dark:bg-red-950/50 dark:text-red-400';
                                        else if (dayMeta?.is_public_holiday || dayMeta?.reason_code === 'special_no_subsidy_day') cellCls = 'bg-amber-100 text-amber-700 dark:bg-amber-950/50 dark:text-amber-400';
                                        else if (dayMeta?.reason_code === 'special_open_day') cellCls = 'bg-purple-100 text-purple-700 dark:bg-purple-950/50 dark:text-purple-400';
                                        else if (dayMeta?.is_available) cellCls = 'bg-orange-50 text-orange-700 font-medium dark:bg-orange-950/40 dark:text-orange-300';
                                        else if (dayMeta !== undefined && !dayMeta.is_open) cellCls = 'bg-gray-100 text-gray-400 dark:bg-slate-800/70 dark:text-slate-600';
                                        else cellCls = 'text-gray-700 hover:bg-gray-50 dark:text-slate-300 dark:hover:bg-slate-800/60';
                                        return (
                                            <div key={dateStr || `c-${idx}`} title={dayMeta?.label}
                                                className={`relative mx-auto my-0.5 flex h-8 w-8 flex-col items-center justify-center rounded-lg text-[13px] transition-colors ${isToday ? 'ring-2 ring-orange-500 ring-offset-1 dark:ring-offset-slate-900' : ''} ${cellCls}`}>
                                                <span className="leading-none">{day}</span>
                                                {isConsumed && (scanCount <= 1
                                                    ? <CheckCircle className={`absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 ${isToday ? 'text-emerald-300' : 'text-emerald-500'}`} />
                                                    : <span className={`absolute -bottom-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full text-[9px] font-bold ${isToday ? 'bg-emerald-300 text-emerald-900' : 'bg-emerald-500 text-white'}`}>{scanCount}</span>
                                                )}
                                            </div>
                                        );
                                    })}
                            ...
                            <div className="border-t border-gray-100 px-4 py-3 dark:border-slate-800">
                                <div className="grid grid-cols-2 gap-x-3 gap-y-1.5">
                                    {[['bg-orange-50 border border-orange-200', t('cafeteria.calendar_available')], ['bg-emerald-500', t('cafeteria.calendar_consumed')], ['bg-gray-100', t('cafeteria.calendar_closed')], ['bg-amber-100', t('cafeteria.calendar_public_holiday')], ['bg-purple-100', t('cafeteria.calendar_special_open_day')], ['bg-red-100', t('cafeteria.calendar_employee_leave')]].map(([color, label]) => (
                                        <span key={label} className="inline-flex items-center gap-1.5 text-[11px] text-gray-500"><span className={`h-2.5 w-2.5 shrink-0 rounded-sm ${color}`} />{label}</span>
                                    ))}
                                </div>
                            </div>
                        </div>
                        ...
                        {scan_result?.employee ? (
                            <div className="rounded-xl border border-gray-200 bg-gray-50 p-5 dark:border-slate-700 dark:bg-slate-950">
                                <div className="flex flex-col items-center text-center">
                                    ...
                                    {scan_result.employee.position && <p className="mt-1 text-xs text-gray-400">{scan_result.employee.position}</p>}
                                </div>
                            </div>
                        ) : (
                            <div className="flex min-h-[180px] flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 bg-gray-50 p-8 text-center dark:border-slate-700 dark:bg-slate-950">
                                ...
                                <p className="text-sm text-gray-400">{t('cafeteria.scanToSeeEmployee')}</p>
                            </div>
                        )}
                    </div>
                                        ...
                                        <li key={scan.id} className="rounded-lg border border-gray-200 bg-white p-3 text-sm dark:border-slate-800 dark:bg-slate-900">
                                            <div className="flex items-start justify-between gap-3">
                                                <div className="min-w-0">
                                                    <p className="truncate font-semibold text-gray-900 dark:text-slate-100">{scan.employee?.display_name ?? t('cafeteria.employeeName')}</p>
                                                    <p className="text-xs text-gray-500">{scan.employee?.employee_number} - {scan.employee?.organization_name ?? ''}</p>
                                                ...
                                                <span className={`shrink-0 rounded-full px-2 py-0.5 text-[11px] font-semibold ${scan.status === 'accepted' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
